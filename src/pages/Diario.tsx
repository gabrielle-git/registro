...
export function Diario() {
  ...
  const entradasFiltradas = useMemo(() => {
    ...
    if (busca.trim()) {
      ...
      lista = lista.filter((e) =>
        e.texto.toLowerCase().includes(termo) ||
  ...
  const resolverNomesSetores = (ids?: string[]): string[] => {
    if (!ids) return []
    return ids
  ...
  const resolverNomesPessoas = (ids?: string[]): string[] => {
    if (!ids) return []
    return ids
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
        <button
          ...
          style={chipFiltroStyle(filtroTipo === 'todos')}
        >
          Todos
        </button>
        {TIPOS_ENTRADA.map((t) => (
          <button
            ...
            style={chipFiltroStyle(filtroTipo === t, CORES_TIPO[t])}
          >
